...
class Scheduler {
    ...
    resolveTime(definition) {
        ...
        switch (definition.type) {
            case 'delay':
                if (typeof definition.milliseconds !== 'number' || definition.milliseconds <= 0) {
                    return null;
                }
    ...
    _resolveSunTime(event, angle) {
        ...
        SunCalc.addTime(angle, `custom${direction}${angle}`, `custom${direction}${angle}`);

        const times = SunCalc.getTimes(new Date(), this.latitude, this.longitude);
        const key = `custom${direction}${angle}`;
    ...
    _isBetween(on, off, now = new Date()) {
        ...
        if (on instanceof Date && !off) {
            return now >= on;
        }

        if (!on && off instanceof Date) {
            return now < off;
        }
